...
mongocxx::pool mongo_pool{mongocxx::uri{"mongodb+srv://xrbams:LmVniuYFmyMpVbGF@xrbams.ni6ob.mongodb.net/?retryWrites=true&w=majority&appName=xrbams"}};
Database::Database() {
        auto conn = mongo_pool.acquire();
        db = (*conn)["datacrawler_xr"];

        std::cout << "MongoDB connection successful in thread " << std::this_thread::get_id() << std::endl;
    }

void Database::saveData(const json& data){
    try{
        auto conn = mongo_pool.acquire();
        auto collection = (*conn)["datacrawler_xr"]["crawled_data"];
...
void Database::saveLink(const std::string& url, const std::string& parentUrl) {
    ...
    {
        auto conn = mongo_pool.acquire();
        auto collection = (*conn)["datacrawler_xr"]["crawled_links"];
        json linkData = {
            {"url", url},
